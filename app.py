...
# --- TAB 1: 新增開團 ---
with tab1:
    # --- 🔥 強制初始化與修復格式 (解決 KeyError: 'id' 的關鍵) ---
    if 'ball_usage' not in st.session_state:
        st.session_state.ball_usage = []
    
    # 如果裡面是舊格式(沒 id)，直接清空重新初始化
    if len(st.session_state.ball_usage) > 0:
        if not isinstance(st.session_state.ball_usage[0], dict) or 'id' not in st.session_state.ball_usage[0]:
            st.session_state.ball_usage = []
    ...
    with st.container(border=True):
        ...
        if not ball_options:
            st.error("❌ 雲端抓不到球種！請檢查 Settings 分頁或按左側修復。")
        else:
            total_ball_cost = 0.0
            ball_details_text = []
            
            # 使用副本進行迴圈避免刪除時出錯
            for index, usage in enumerate(list(st.session_state.ball_usage)):
                uid = usage.get("id")
                if not uid: continue # 雙重保險
                ...
                with r1:
                    ...
                    usage['ball_type'] = st.selectbox(f"球種", ball_options, index=idx, key=f"sel_{uid}", label_visibility="collapsed")
                
                with r2:
                    usage['count'] = st.number_input(f"顆數", min_value=0, step=1, value=usage.get('count', 0), key=f"num_{uid}", label_visibility="collapsed")
            ...
            if st.button("➕ 新增球種項目"):
                st.session_state.ball_usage.append({"id": str(time.time() + len(st.session_state.ball_usage)), "ball_type": ball_options[0], "count": 0})
                st.rerun()
...
with tab3:
    ...
    if not records_df.empty:
        ...
        idx = st.number_input("輸入列號刪除", min_value=1, max_value=len(records_df), step=1, key="del_rec_idx")
        if st.button("🗑️ 刪除紀錄"):
